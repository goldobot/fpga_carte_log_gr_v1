...
void edit_input_buf ()
{
  ...
  volatile int* leds_reg = ( volatile int* ) &LEDS_BASE_FAKE;
...
int main () {
    ...
    //volatile int* leds_reg = ( volatile int* ) LEDS_BASE_ADDR;
    volatile int* leds_reg = ( volatile int* ) &LEDS_BASE_FAKE;
    volatile int* true_leds_reg = ( volatile int* ) LEDS_BASE_ADDR;
    unsigned int mask = 0xff;
    unsigned int leds = mask;
    uint32_t loop_cnt=0;
    ...
    uart_putchar ( 0xa );
    uart_putstring ( "Robot GOLDO - soft DEBUG 2020" );
    uart_putchar ( 0xa );
    ...
    for (;;) {
      ...
      *leds_reg = leds & 0xff;
      ...
      /* FIXME : DEBUG */
      {
        unsigned int dbg_leds = 0;
        dbg_leds = robot_reg[0x13f];
        *true_leds_reg = dbg_leds & 0xff;
      }
