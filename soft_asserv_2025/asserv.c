...
void init_asserv (struct _goldo_asserv *_ga, uint32_t _mot_reg, uint32_t _enc_reg,
                  ...
                  uint32_t _sw_reg, uint32_t _sw_mask, int _home_dir, int _polar)
{
...
void do_step_asserv (struct _goldo_asserv *_ga)
{
  ...
  else
  {
    if (asserv_state_is (_ga, GA_STATE_HOMING))
    {
      ...
      robot_reg[_ga->mot_reg] = 0x00000020;
    }
    ...
    if ((my_val32&sw_mask)==sw_mask) /* /!\ HW 2025 : 0=OFF ; 1=ON  */
    ...
    {
      robot_reg[_ga->mot_reg] = 0x00000000;
      //uart_putchar ( '.' );
      //uart_putchar ( 0xa );
      if (asserv_state_is (_ga, GA_STATE_HOMING))
      {
